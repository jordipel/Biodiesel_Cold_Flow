...
Propiedad='Pour Point [ºC]'
#Propiedad='Cold filter plugging point [C]'
#Propiedad='Cloud Point [C]'
...
# Tipo 1 = Trabajamos con FAME

# Tipo 2 = Trabajamos con FAME y Others 

# Tipo 3 = Trabajamos con FAME y MUFA

# Tipo 4 = Trabajamos con FAME y DB

# Tipo 5 = Trabajamos con FAME y Cn

# Tipo 6 = Trabajamos con FAME y Mw

# Tipo 7 = Trabajamos con FAME y NPI

# Tipo 8 = Trabajamos con parámetros de Mustafa
...
if tipo==1:
    ...
    tipo_s = ['Others','NPI', 'DB' ,'Mw','Cn']    # FAME y MUFA

    descuenta = -9 + len(tipo_s)

elif tipo==4:

    tipo_s = ['Others','MUFA','PUFA','FSA','NPI' ,'Mw','Cn']    # FAME y DB

    descuenta = -9 + len(tipo_s) # -9+7=2 quita en la suma los 2 que hay a partir de los Cs (1 + la clase)

elif tipo==5:

    tipo_s = ['Others','MUFA','PUFA','FSA','NPI' ,'Mw','DB']    # FAME y Cn

    descuenta = -9 + len(tipo_s) # -9+7=2 quita en la suma los 2 que hay a partir de los Cs (1 + la clase)

elif tipo==6:

    tipo_s = ['Others','MUFA','PUFA','FSA','NPI' ,'Cn','DB']    # FAME y Mw

    descuenta = -9 + len(tipo_s) # -9+7=2 quita en la suma los 2 que hay a partir de los Cs (1 + la clase)

elif tipo==7:

    tipo_s = ['Others','MUFA','PUFA','FSA','Mw' ,'Cn','DB']    # FAME y NPI
    ...
    tipo_s = ['C4:0', 'C6:0', 'C13:0', 'C14:1', 'C15:0', 'C15:1', 'C16:1', 'C16:2', 'C16:3', 'C16:4', 
              ...
              'C21:1', 'C22:0', 'C22:5', 'C22:6', 'C24:0', 'Others']
...
'''
tipo_s = ['C4:0', 'C6:0', 'C10:0','C12:0','C14:0','C16:0','C18:0','C18:1','C18:2','C20:0','C22:3', 'C14:1', 'C15:0', 'C15:1', 'C16:1', 'C16:2', 'C16:3', 'C16:4', 

              'C17:0', 'C17:1', 'C19:0', 'C19:1', 'C19:3', 'C20:2', 'C20:4', 'C20:5', 'C21:0', 
...
if RUN_DecTree:
    ...
    valores_max_depth = [2, 3, 4, 5, 6]

    hyperparameters = dict(max_depth=valores_max_depth)

    # Mejor combinación Cetane con solo FAME max_depth:5
...
if RUN_RandomForest:
    ...
    valores_n_estimators = [ 100, 150, 175]

    valores_max_depth = [ 5, 6, 7]

    #valores_n_estimators = [ 100]
    #valores_max_depth = [3]


    hyperparameters = dict(n_estimators=valores_n_estimators, max_depth=valores_max_depth)

    # Mejor combinación Cetane con solo FAME max_depth:6 n_estimators:150
    # Mejor combinación Cetane con FAME y MUFA max_depth:7 n_estimators:175
    # Mejor combinación Cetane con FAME y DB max_depth:7 n_estimators:175
    ...
    sys_impor = RandomForestRegressor(random_state=1234,n_estimators = 150,max_depth=5 )
    sys_impor.fit(X,y)
    import_names = sys_impor.feature_names_in_
    importance = sys_impor.feature_importances_
    '''
...
if RUN_XGBoost:
    ...
    valores_n_estimators = [  100, 150, 200]

    valores_max_depth = [4, 5, 6]

    valores_learning_rate = [ 0.2, 0.1, 0.01]

    hyperparameters = dict(n_estimators=valores_n_estimators, max_depth=valores_max_depth, learning_rate=valores_learning_rate)

    # Mejor combinación Cetane con solo FAME max_depth:5 n_estimators:150 learning_rate: 0,2
    ...
    resultados = np.vstack([resultados, [mae,mse,r2,tiempo,mae_rees,mse_rees,r2_rees]])
    '''
    sys_impor = XGBRegressor(random_state=1234,n_estimators = 100,max_depth=5, learning_rate = 0.1 )
    sys_impor.fit(X,y)
    import_names = sys_impor.feature_names_in_
    importance = sys_impor.feature_importances_
...
if RUN_STD_SRV_LINEAR:
    ...
    valores_Cs = [0.01, 0.1, 1, 10, 50, 100]

    hyperparameters = dict(svr__C=valores_Cs)

    # Mejor combinación Cetane con solo FAME Cs:0.1
...
if RUN_STD_SRV_POLY:
    ...
    valores_Cs = [0.0001,0.001,0.01, 0.1, 1]

    valores_degrees = [1, 2, 3]   

    hyperparameters = dict(svr__C=valores_Cs, svr__degree=valores_degrees)

    # Mejor combinación Cetane con solo FAME Cs:0.001 degree:2
...
if RUN_ANN:
    ...
    #sys_ANN = MLPRegressor(random_state=1234, solver='sgd', tol=0.1, batch_size=32, max_iter=2000)
   
    #sys = Pipeline([('std', StandardScaler()), ('ANN', sys_ANN)])

    sys = MLPRegressor(random_state=1234, solver='sgd', tol=0.1, batch_size=32, max_iter=2000)

    # se definen los valores de los hiperparámetros que se quieren probar y se introducen en un diccionario

    valores_capas = [(16,), (32,), (48,)]
    ...
    # Mejor combinación Cetane con solo FAME capas:(32,) lr_init:0.00001
...
if RUN_ANN_brasil:
    ...
    sys = MLPRegressor(random_state=1234, hidden_layer_sizes=(6,), n_iter_no_change=20, solver='sgd', 
